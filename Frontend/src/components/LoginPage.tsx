...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-4 relative overflow-hidden ${
      isDarkMode ? 'bg-[#121212] text-white' : 'bg-slate-50 text-slate-900'
    }`}>
      {/* Background Glows */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#1ed760]/10 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-emerald-500/10 rounded-full blur-[120px] pointer-events-none" />

      <div className={`max-w-md w-full rounded-3xl border p-8 shadow-2xl relative z-10 transition-colors ${
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={loading}
            className="w-full py-3.5 px-6 rounded-full bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs uppercase tracking-wider shadow-lg transition flex items-center justify-center space-x-3 cursor-pointer disabled:opacity-50"
          >
            {loading ? (
        ...
        <div className={`mt-8 pt-6 border-t text-center text-[10px] ${isDarkMode ? 'border-[#282828] text-[#b3b3b3]' : 'border-slate-200 text-slate-500'}`}>
          Secure Google Authentication & Firestore Cloud Persistence
        </div>
      </div>
    </div>
  );
