...
export const ApiExplorer: React.FC = () => {
  ...
  const endpoints: ExplorerEndpoint[] = [
    ...
    { category: 'CONSISTENCY', method: 'POST', path: '/consistency/audit', desc: 'Audit outputs against Fact Registry for a source_id', defaultPayload: { source_id: "src_demo_01", outputs: { summary: "Revenue reached $50M in Q4 [F001]." } }, timeout: 60000 },
    { category: 'CONSISTENCY', method: 'POST', path: '/consistency/validate', desc: 'Detailed 6-dimensional consistency validation scores', defaultPayload: { uckr: { source_id: "src_01", facts: [{ fact_id: "F001", statement: "Revenue reached $50M" }] }, outputs: { summary: "Revenue reached $50M [F001]." } }, timeout: 60000 },
    { category: 'CONSISTENCY', method: 'POST', path: '/consistency/repair', desc: 'Automatic Repair Engine for numeric/entity discrepancies', defaultPayload: { uckr: { source_id: "src_01", facts: [{ fact_id: "F001", statement: "Revenue reached $50M" }] }, outputs: { summary: "Revenue reached $95M." } }, timeout: 120000 },
    { category: 'CONSISTENCY', method: 'POST', path: '/consistency/diff', desc: 'Diff two UCKR versions to detect added/changed facts', defaultPayload: { v1: { source_id: "src_01", facts: [{ fact_id: "F001", statement: "Revenue $50M" }] }, v2: { source_id: "src_01", facts: [{ fact_id: "F001", statement: "Revenue $55M" }] } }, timeout: 60000 },
    { category: 'CONSISTENCY', method: 'POST', path: '/consistency/regenerate-affected', desc: 'Incremental selective regeneration for affected channels only', defaultPayload: { uckr_v2: { source_id: "src_01", facts: [{ fact_id: "F001", statement: "Revenue $55M" }] }, diff: { added_facts: [], changed_facts: ["F001"], deleted_facts: [], affected_channels: ["summary"] }, previous_outputs: { summary: "Revenue $50M", linkedin: "Unchanged post" } }, timeout: 180000 },
    { category: 'CONSISTENCY', method: 'POST', path: '/consistency/evidence', desc: 'Trace every generated statement to Fact IDs, source text, pages + verification', defaultPayload: { uckr: { document: { id: "SRC-01", title: "Q4 Report" }, core_topic: "Revenue", summary: "Revenue update.", facts: [{ id: "F001", statement: "Revenue reached $50M in Q4.", importance: 0.95, source_reference: "page_1", confidence: 0.9 }] }, outputs: { summary: { text: "Revenue reached $50M in Q4.", source_facts: ["F001"] } } }, timeout: 60000 },
    { category: 'CONSISTENCY', method: 'POST', path: '/consistency/translate', desc: 'Translate UCKR + deliverables into Indian/global languages', defaultPayload: { target_language: "Hindi", outputs: { summary: { text: "Revenue reached $50M." } } }, timeout: 180000 },
    { category: 'CONSISTENCY', method: 'GET', path: '/consistency/languages', desc: 'List supported translation languages + TTS voices', defaultPayload: {} },
