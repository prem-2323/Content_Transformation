...
export const ContentIntelligence: React.FC<ContentIntelligenceProps> = ({ transformationResult }) => {
  ...
  const validationReport = repairedData?.final_validation_report || transformationResult?.validation_report || {
    passed: true,
    overall_score: 95,
    breakdown: {
      fact_consistency: 98,
  ...
  const outputs = repairedData?.repaired_outputs || transformationResult?.outputs || {
    summary: "Executive summary grounded in UCKR [F001].",
    linkedin: "LinkedIn post draft [F001]."
  };
  ...
  // Extract detected inconsistency categories
  const violations = validationReport.violations || [];
  
  const numericConflicts = violations.filter((v: any) => 
    v.type === 'numeric_mismatch' || v.message?.toLowerCase().includes('percentage') || v.message?.toLowerCase().includes('number')
  );

  const dateConflicts = violations.filter((v: any) => 
    v.type === 'date_mismatch' || v.message?.toLowerCase().includes('date') || v.message?.toLowerCase().includes('year') || v.message?.toLowerCase().includes('q4')
  );

  const missingFacts = violations.filter((v: any) => 
    v.type === 'missing_fact' || v.message?.toLowerCase().includes('weakly represented') || v.message?.toLowerCase().includes('missing')
  );

  const changedNames = violations.filter((v: any) => 
    v.type === 'entity_mismatch' || v.message?.toLowerCase().includes('entity') || v.message?.toLowerCase().includes('name')
  );
